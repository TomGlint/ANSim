...
VC::VC(const Configuration &config, long long int outputs,
      ...
      _expected_pid(-1), _last_id(-1), _last_pid(-1)
{
  ...
  _priority_donation = config.GetLongInt("vc_priority_donation");
}
...
void VC::AddFlit(Flit *f)
{
  ...
  {
    f->pri = numeric_limits<long long int>::max() - GetSimTime();
    assert(f->pri >= 0);
  }
...
void VC::SetOutput(long long int port, long long int vc)
{
  _out_port = port;
  _out_vc = vc;
}
...
void VC::Route(tRoutingFunction rf, const Router *router, const Flit *f, long long int in_channel)
{
