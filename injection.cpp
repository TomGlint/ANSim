...
InjectionProcess::InjectionProcess(long long int nodes, double rate)
...
InjectionProcess *InjectionProcess::New(string const &inject, long long int nodes,
                                        double load,
                                        Configuration const *const config)
{
  ...
  else if (process_name == "on_off")
  {
    ...
    vector<long long int> initial(nodes);
    if (params.size() > 3)
    {
    ...
    {
      for (long long int n = 0; n < nodes; ++n)
      {
...
BernoulliInjectionProcess::BernoulliInjectionProcess(long long int nodes, double rate)
    : InjectionProcess(nodes, rate)
{
}

bool BernoulliInjectionProcess::test(long long int source)
{
...
OnOffInjectionProcess::OnOffInjectionProcess(long long int nodes, double rate,
...
bool OnOffInjectionProcess::test(long long int source)
{
  assert((source >= 0) && (source < _nodes));
