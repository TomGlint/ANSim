...
#define mod_sum(x, y) (((x) + (y)) - (long long int)((x) + (y))) /* (x+y) mod 1.0 */

double ran_u[KK]; /* the generator state */

#ifdef __STDC__
void ranf_array(double aa[], long long int n)
#else
void ranf_array(aa, n) /* put n new random fractions in aa */
    double *aa;        /* destination */
long long int n;       /* array length (must be at least KK) */
#endif
{
