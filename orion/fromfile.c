...
int Flexus_Orion_init(const Configuration &config)
{
  ...
  PARM_in_port = config.GetLongInt("Orion_inport");      //5;
  PARM_out_port = config.GetLongInt("Orion_outport");    //5;
  PARM_flit_width = config.GetLongInt("Orion_bitwidth"); //64;
  ...
  PARM_v_class = config.GetLongInt("Orion_vc_class"); //1;   /* # of total message classes */
  PARM_v_channel = config.GetLongInt("num_vcs");      /* # of virtual channels per virtual message class*/

  //printf("param  fromfile: %g\n", PARM_Vdd);
  //exit(0);

  PARM_in_share_buf = config.GetLongInt("Orion_IsSharedBuffIn");   // 0;	/* do input virtual channels physically share buffers? */
  PARM_out_share_buf = config.GetLongInt("Orion_IsSharedBuffOut"); //0;	/* do output virtual channels physically share buffers? */
  PARM_crossbar_model = config.GetLongInt("Orion_crossbar_model"); //MULTREE_CROSSBAR;	/* crossbar model type MATRIX_CROSSBAR, MULTREE_CROSSBAR, or TRISTATE_CROSSBAR (only for Orion3.0)*/
  PARM_crsbar_degree = config.GetLongInt("Orion_crsbar_degree");   //4;					/* crossbar mux degree */
  PARM_connect_type = config.GetLongInt("Orion_Cxbar_Cxpoint");    //TRISTATE_GATE;		/* crossbar connector type */
  PARM_trans_type = config.GetLongInt("Orion_trans_type");         //NP_GATE;				/* crossbar transmission gate type */
  PARM_in_buf = config.GetLongInt("Orion_IsInBuff");               //1;//			1		/* have input buffer? */
  PARM_in_buf_set = config.GetLongInt("vc_buf_size");              //8;//		1

  PARM_in_buffer_type = config.GetLongInt("Orion_buff_type"); //SRAM ;//SRAM	/*buffer model type, SRAM or REGISTER*/

  PARM_out_buf = config.GetLongInt("Orion_IsOutBuff");                 //0	;//		0
  PARM_out_buf_set = config.GetLongInt("Orion_out_buf_size");          //0;//		2
  PARM_out_buffer_type = config.GetLongInt("Orion_buff_type");         //SRAM;//  	SRAM		/*buffer model type, SRAM or REGISTER*/
  PARM_sw_in_arb_model = config.GetLongInt("Orion_in_arb_model");      //RR_ARBITER;//		/* input side arbiter model type, MATRIX_ARBITER , RR_ARBITER, QUEUE_ARBITER*/
  PARM_sw_out_arb_model = config.GetLongInt("Orion_out_arb_model");    //RR_ARBITER;//	/* output side arbiter model type, MATRIX_ARBITER */
  PARM_vc_allocator_type = config.GetLongInt("Orion_allocator_model"); //TWO_STAGE_ARB;//	/*vc allocator type, ONE_STAGE_ARB, TWO_STAGE_ARB, VC_SELECT*/
  PARM_vc_in_arb_model = config.GetLongInt("Orion_in_vc_arb_model");   //RR_ARBITER;//    /*input side arbiter model type for TWO_STAGE_ARB. MATRIX_ARBITER, RR_ARBITER, QUEUE_ARBITER*/
  PARM_vc_out_arb_model = config.GetLongInt("Orion_out_vc_arb_model"); //RR_ARBITER ;// 	/*output side arbiter model type (for both ONE_STAGE_ARB and TWO_STAGE_ARB). MATRIX_ARBITER, RR_ARBITER, QUEUE_ARBITER */
