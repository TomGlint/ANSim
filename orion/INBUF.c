...
int INBUF_initialize(INBUF *inbuffer)
{
	...
	inbuffer->insts = 180 * inbuffer->p_in * inbuffer->v_channel + 2 * inbuffer->p_in * inbuffer->v_channel * inbuffer->buf_in * inbuffer->flit_width + 2 * inbuffer->p_in * inbuffer->p_in * inbuffer->v_channel * inbuffer->buf_in + 3 * inbuffer->p_in * inbuffer->v_channel * inbuffer->buf_in + 5 * inbuffer->p_in * inbuffer->p_in * inbuffer->buf_in + inbuffer->p_in * inbuffer->p_in + inbuffer->p_in * inbuffer->flit_width +
					  15 * inbuffer->p_in;
...
double INBUF_get_internal_power(void *inbuffer)
{
	...
	return (AOI_int_J + DFF_int_J) * .5 * (((INBUF *)inbuffer)->insts * ((INBUF *)inbuffer)->tr + .05 * ((INBUF *)inbuffer)->insts);
}
...
double INBUF_get_switching_power(void *inbuffer)
{
	...
	return 0.5 * 1.4 * PARM(VDD_V) * PARM(VDD_V) * ((INBUF *)inbuffer)->clk * .5 *
		   (((INBUF *)inbuffer)->insts * ((INBUF *)inbuffer)->tr * AOI_load_pF + .05 * ((INBUF *)inbuffer)->insts * DFF_load_pF);
}
