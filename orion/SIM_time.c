...
double SIM_compare_time(int C, int A, int Ntbl, int Ntspd, double inputtime, double *outputtime)
{
	...
	c1 = (tagbits) * (SIM_draincap(Wcompn, NCH, 1) + SIM_draincap(Wcompn, NCH, 2)) + SIM_draincap(Wcompp, PCH, 1) + SIM_gatecap(Wmuxdrv12n + Wmuxdrv12p, 20.0) +
		 cols * Cwordmetal;
				...
				for (Ntspd = 1; Ntspd <= PARM(MAXSPD); Ntspd = Ntspd * 2)
				{
					for (Ntwl = 1; Ntwl <= 1; Ntwl = Ntwl * 2)
					{
						for (Ntbl = 1; Ntbl <= PARM(MAXN); Ntbl = Ntbl * 2)
						{
							...
							if (SIM_organizational_parameters_valid(rows, columns, Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd))
							{
