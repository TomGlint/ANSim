...
int SWVC_initialize(SWVC *arbiter)
{
	...
	arbiter->insts = static_cast<int>(9 * ((pow(arbiter->p_in, 2) * arbiter->v_channel *
											arbiter->v_channel) +
										   pow(arbiter->p_in, 2) + (arbiter->p_in * arbiter->v_channel) - arbiter->p_in));
...
{
	return (6 * NOR_int_J + 2 * INV_int_J + DFF_int_J) * ((SWVC *)arbiter)->insts * ((SWVC *)arbiter)->tr;
}

double SWVC_get_switching_power(void *arbiter)
{
	return 0.5 * 1.4 * (NOR_load_pF + INV_load_pF + DFF_load_pF) * PARM(VDD_V) * PARM(VDD_V) * ((SWVC *)arbiter)->clk *
