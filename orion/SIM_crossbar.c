...
static double SIM_crossbar_in_cap(double wire_cap, u_int n_out, double n_seg, int connect_type, int trans_type, double *Nsize)
{
	...
	if (n_seg > 1)
	{
		...
		Ctotal += (n_seg + 2) * (n_seg - 1) / (n_seg * 2) * (SIM_gatecap(Woutdrvnandn + Woutdrvnandp, 0) + SIM_gatecap(Woutdrvnorn + Woutdrvnorp, 0));
		/* output capacitance of tri-state buffer */
		Ctotal += (n_seg - 1) / 2 * (SIM_draincap(Woutdrivern, NCH, 1) + SIM_draincap(Woutdriverp, PCH, 1));
	}
