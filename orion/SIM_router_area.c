...
int SIM_router_area_init(SIM_router_info_t *info, SIM_router_area_t *router_area)
{
	...
	if (info->in_buf)
	{
		...
		case SRAM:
			...
			wordline_len = info->flit_width * (RegCellWidth + 2 * (info->in_buf_info.read_ports + info->in_buf_info.write_ports) * BitlineSpacing);
	...
	if (info->out_buf)
	{
		...
		case SRAM:
			...
			wordline_len = info->flit_width * (RegCellWidth + 2 * (info->out_buf_info.read_ports + info->out_buf_info.write_ports) * BitlineSpacing);
	...
	if (info->sw_in_arb_model)
	{
		...
		case MATRIX_ARBITER: //assumes 30% spacing for each arbiter
			router_area->sw_allocator += ((AreaNOR * 2 * (req_width - 1) * req_width) + (AreaINV * req_width) + (AreaDFF * (req_width * (req_width - 1) / 2))) * 1.3 * info->in_n_switch * info->n_in;
			break;

		case RR_ARBITER: //assumes 30% spacing for each arbiter
			router_area->sw_allocator += ((6 * req_width * AreaNOR) + (2 * req_width * AreaINV) + (req_width * AreaDFF)) * 1.3 * info->in_n_switch * info->n_in;
			break;

		case QUEUE_ARBITER:
			router_area->sw_allocator += AreaDFF * info->sw_in_arb_queue_info.n_set * info->sw_in_arb_queue_info.data_width * info->in_n_switch * info->n_in;
	...
	if (info->sw_out_arb_model)
	{
		...
		{
		case MATRIX_ARBITER: //assumes 30% spacing for each arbiter
			router_area->sw_allocator += ((AreaNOR * 2 * (req_width - 1) * req_width) + (AreaINV * req_width) + (AreaDFF * (req_width * (req_width - 1) / 2))) * 1.3 * info->n_switch_out;
			break;
		...
		case QUEUE_ARBITER:
			router_area->sw_allocator += AreaDFF * info->sw_out_arb_queue_info.data_width * info->sw_out_arb_queue_info.n_set * info->n_switch_out;
			break;
	...
	if (info->vc_allocator_type == ONE_STAGE_ARB && info->n_v_channel > 1 && info->n_in > 1)
	{
		if (info->vc_out_arb_model)
		{
			...
			{
			case MATRIX_ARBITER: //assumes 30% spacing for each arbiter
				router_area->vc_allocator = ((AreaNOR * 2 * (req_width - 1) * req_width) + (AreaINV * req_width) + (AreaDFF * (req_width * (req_width - 1) / 2))) * 1.3 * info->n_out * info->n_v_channel * info->n_v_class;
				break;

			case RR_ARBITER: //assumes 30% spacing for each arbiter
				router_area->vc_allocator = ((6 * req_width * AreaNOR) + (2 * req_width * AreaINV) + (req_width * AreaDFF)) * 1.3 * info->n_out * info->n_v_channel * info->n_v_class;
				break;

			case QUEUE_ARBITER:
				router_area->vc_allocator = AreaDFF * info->vc_out_arb_queue_info.data_width * info->vc_out_arb_queue_info.n_set * info->n_out * info->n_v_channel * info->n_v_class;
	...
	else if (info->vc_allocator_type == TWO_STAGE_ARB && info->n_v_channel > 1 && info->n_in > 1)
	{
		if (info->vc_in_arb_model && info->vc_out_arb_model)
		{
			...
			{
			case MATRIX_ARBITER: //assumes 30% spacing for each arbiter
				router_area->vc_allocator = ((AreaNOR * 2 * (req_width - 1) * req_width) + (AreaINV * req_width) + (AreaDFF * (req_width * (req_width - 1) / 2))) * 1.3 * info->n_in * info->n_v_channel * info->n_v_class;
				break;

			case RR_ARBITER: //assumes 30% spacing for each arbiter
				router_area->vc_allocator = ((6 * req_width * AreaNOR) + (2 * req_width * AreaINV) + (req_width * AreaDFF)) * 1.3 * info->n_in * info->n_v_channel * info->n_v_class;
				break;

			case QUEUE_ARBITER:
				router_area->vc_allocator = AreaDFF * info->vc_in_arb_queue_info.data_width * info->vc_in_arb_queue_info.n_set * info->n_in * info->n_v_channel * info->n_v_class;
	...
	else if (info->vc_allocator_type == VC_SELECT && info->n_v_channel > 1)
	{
		...
		case SRAM:
			...
			wordline_len = SIM_logtwo(info->n_v_channel) * (RegCellWidth + 2 * (info->vc_select_buf_info.read_ports + info->vc_select_buf_info.write_ports) * BitlineSpacing);
			router_area->vc_allocator = info->n_out * info->n_v_class * (bitline_len * wordline_len);

			break;

		case REGISTER:
			router_area->vc_allocator = AreaDFF * info->n_out * info->n_v_class * info->vc_select_buf_info.data_width * info->vc_select_buf_info.n_set;
