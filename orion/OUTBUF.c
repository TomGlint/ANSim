...
double OUTBUF_get_internal_power(void *outbuffer)
{
	...
	return (AOI_int_J + DFF_int_J) * .5 * (((OUTBUF *)outbuffer)->insts * ((OUTBUF *)outbuffer)->tr + .05 * ((OUTBUF *)outbuffer)->insts);
}

double OUTBUF_get_switching_power(void *outbuffer)
{

	return 0.5 * 1.4 * PARM(VDD_V) * PARM(VDD_V) * ((OUTBUF *)outbuffer)->clk * .5 * (((OUTBUF *)outbuffer)->insts * ((OUTBUF *)outbuffer)->tr * AOI_load_pF + .05 * ((OUTBUF *)outbuffer)->insts * DFF_load_pF);
}
