...
int CLKCTRL_initialize(CLKCTRL *clockctrl)
{
	...
	clockctrl->insts = static_cast<int>(.02 * ((25 * clockctrl->p_out + 80 * clockctrl->p_out *
											   ...
											   (180 * clockctrl->p_in *
													clockctrl->v_channel +
												2 * clockctrl->p_in * clockctrl->v_channel * clockctrl->buf_in *
													clockctrl->flit_width +
												2 * clockctrl->p_in *
												...
												clockctrl->p_in * clockctrl->p_in + clockctrl->p_in * clockctrl->flit_width + 15 * clockctrl->p_in) +
											   9 * ((pow(clockctrl->p_in, 2) * clockctrl->v_channel *
													 clockctrl->v_channel) +
													pow(clockctrl->p_in, 2) + (clockctrl->p_in * clockctrl->v_channel) - clockctrl->p_in)));
...
{
	return (AOI_int_J + INV_int_J) * ((CLKCTRL *)clockctrl)->insts * ((CLKCTRL *)clockctrl)->tr;
}

double CLKCTRL_get_switching_power(void *clockctrl)
{
	return .5 * 1.4 * (INV_load_pF + AOI_load_pF) * PARM(VDD_V) * PARM(VDD_V) *
		   ((CLKCTRL *)clockctrl)->clk * ((CLKCTRL *)clockctrl)->insts * ((CLKCTRL *)clockctrl)->tr;
}
