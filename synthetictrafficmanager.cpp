    ...
    : TrafficManager(config, net)
{
  ...
  {
    _packet_size.push_back(vector<long long int>(1, config.GetLongInt("packet_size")));
  }
  else
  {
  ...
  {
    long long int rate = config.GetLongInt("packet_size_rate");
    assert(rate >= 0);
    for (long long int c = 0; c < _classes; ++c)
    {
  ...
  {
    _reply_class.push_back(config.GetLongInt("reply_class"));
  }
