...
{
  //glint
  asyncConfig->netraceInterCycle = (asyncConfig->netraceInterCycle + 1) % asyncConfig->traceStretch;
  ...
  for (unsigned long long int s = 0; s < _scale; ++s)
  {
    if (asyncConfig->netraceInterCycle == 0)
    {
      ++_time;
    }
...
long long int NetraceWorkload::time() const
{
  ...
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const time = (long long int)(_ready_packets[source].front()->cycle * asyncConfig->traceStretch / _scale);
  //assert(time >= 0);
  return time;
}
